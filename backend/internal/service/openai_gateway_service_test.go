...
func TestOpenAIStreamingFirstOutputTimeoutIgnoresPreambleAndKeepalive(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize:              defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Header: http.Header{
			...
			"X-Request-Id":     []string{"rid-first-output-timeout"},
		},
	}
	preamble := fmt.Sprintf(
		"event: response.in_progress\ndata: {\"type\":\"response.in_progress\",\"padding\":%q}\n\n: upstream-heartbeat\n\n",
	...
	go func() {
		...
		defer ticker.Stop()
		for {
			if _, err := io.WriteString(pw, preamble); err != nil {
				return
			}
			<-ticker.C
		}
...
func TestOpenAIStreamLineStopsFirstOutputTimer(t *testing.T) {
	tests := []struct {
		...
		{name: "comment", line: ": heartbeat", want: false},
		{name: "event field", line: "event: response.output_text.delta", want: false},
		{name: "created", line: `data: {"type":"response.created"}`, want: false},
		{name: "in progress", line: `data: {"type":"response.in_progress"}`, want: false},
		{name: "failed", line: `data: {"type":"response.failed"}`, want: true},
		{name: "done marker", line: "data: [DONE]", want: true},
		{name: "text delta", line: `data: {"type":"response.output_text.delta","delta":"ready"}`, want: true},
		{name: "completed", line: `data: {"type":"response.completed"}`, want: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, openAIStreamLineStopsFirstOutputTimer(tt.line))
		})
	}
}
...
func TestOpenAIStreamingPassthroughFirstOutputTimeoutIgnoresPreamble(t *testing.T) {
	...
	resp := &http.Response{
		...
		Header: http.Header{
			"Content-Encoding": []string{"gzip"},
			"X-Request-Id":     []string{"rid-pass-first-output-timeout"},
		},
	}
	go func() {
		ticker := time.NewTicker(100 * time.Millisecond)
		defer ticker.Stop()
		for {
			if _, err := io.WriteString(pw, "event: response.created\ndata: {\"type\":\"response.created\"}\n\n: upstream-heartbeat\n\n"); err != nil {
				return
			}
			<-ticker.C
		}
...
func TestOpenAIStreamingFirstOutputTimeoutStopsAfterEffectiveOutput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize:              defaultMaxLineSize,
	...
	go func() {
		...
		_, _ = io.WriteString(pw, "data: {\"type\":\"response.completed\",\"response\":{\"usage\":{\"input_tokens\":1,\"output_tokens\":1}}}\n\n")
	}()
...
func TestOpenAIStreamingPassthroughFirstOutputTimeoutStopsAfterEffectiveOutput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize:              defaultMaxLineSize,
